...
func (p *Peer) handleConnection(conn net.Conn) {
    ...
    // Leer nombre del archivo (máx 256 bytes)
    name := make([]byte, 256)
...
func (p *Peer) SendFile(filePath, toAddress string) error {
    ...
    _, fileName := filepath.Split(filePath) // ✅ Correcto aquí
...
func registrarDelete(file string, fromPeer string) {
    op := logfs.Operation{
        Type:      "DELETE",
        FileName:  file,
        From:      fromPeer,
    ...
    err := logfs.AppendOperation(op)
    if err != nil {
        log.Println("Error al registrar operación:", err)
    }
}
